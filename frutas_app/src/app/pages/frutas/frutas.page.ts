...
export class FrutasPage implements OnInit {
  ...
  agregarFruta() {
    ...
    this.frutasService.createFruta(this.nueva_fruta).subscribe(fruta => {
      this.frutas.push(fruta);
      this.nueva_fruta = { id_fruta: 0, nombre: ''}; // Limpiar el formulario
    });
  }
  ...
  actualizarFruta(id_fruta: number) {
    const fruta = this.frutas.find(f => f.id_fruta === id_fruta);
    if (fruta) {
      this.frutasService.updateFruta(id_fruta, fruta).subscribe();
    }
  }
}*/
